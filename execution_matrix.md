...
Esta matriz converte `codex.md`, `roadmap.md` e `master_schema.md` em execucao real.
...
| A3 | P0 | foundation | schema-part-spine | Formalizar espinha obrigatoria por Part no conteudo real | `master_schema.md` pronto | done | manter a espinha das reality classes como baseline e puxar overkills antigos para o mesmo idioma | Parts presentes seguem um padrao reconhecivel | crescimento continua artesanal |
| A4 | P0 | media-governance | metadata-minimum | Fixar campos minimos de metadado para midia e superficies | `master_schema.md` pronto | done | manter `docs/METADATA_MINIMUM.md` e os registros JSON como baseline canonico | novos assets entram com id, dominio, alvo e status | caos de acervo e retrabalho |
